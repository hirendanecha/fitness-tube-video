...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'tube-dist/video-fitnesstrainer-tube/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'tube-dist/video-fitnesstrainer-tube/browser'), 'index.html')
    )
    .toString();
